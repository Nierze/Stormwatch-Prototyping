...
#cat_img = np.concatenate([rgb, pred_color, target_color], axis=0) # Original merge logic
    #plt.imsave(os.path.join(save_dir, filename[:-4]+'_merge.jpg'), cat_img)
    # plt.imsave(os.path.join(save_dir, filename[:-4]+'_pred.jpg'), pred_color)
    cv2.imwrite(os.path.join(save_dir, filename[:-4]+'_pred.jpg'), pred_color[:, :, ::-1])
...
#cat_img = np.concatenate([rgb_color, pred_color, targ_color], axis=0)
    # plt.imsave(os.path.join(save_dir, filename[:-4]+'_normal.jpg'), cat_img)
    # plt.imsave(os.path.join(save_dir, filename[:-4]+'_normal.jpg'), pred_color)
    cv2.imwrite(os.path.join(save_dir, filename[:-4]+'_normal.jpg'), pred_color[:, :, ::-1])
...
def create_html(name2path, save_path='index.html', size=(256, 384)):
    # Disabled to remove html4vision dependency
    pass
    """
    # table description
    cols = []
